...
with tab3:
    ...
    * 🔴 **停損 (保護本金)：** 買進後，若帳面虧損達 10%，立刻停損出場。
    ...
    if st.button("⏳ 啟動 5 年歷史回測", type="primary"):
        with st.spinner(f"正在向歷史資料庫調閱 {target_stock_bt} 過去 5 年的每一筆交易..."):
            stock_id = f"{target_stock_bt}.TW" if len(target_stock_bt) == 4 else target_stock_bt
            df_bt = get_stock_history(stock_id, period="5y")
            
            if df_bt.empty:
                st.error("⚠️ 抓不到這檔股票的資料，可能是代號錯誤或上市未滿 5 年。")
            else:
                # 重新計算 5 年長度的技術指標
                df_bt = calculate_indicators(df_bt.copy(), kd_days, macd_fast, macd_slow, bb_days, bb_std)
                df_bt = df_bt.dropna() # 移除剛開始算不出平均線的空白天數
                ...
                for date, row in df_bt.iterrows():
                    if not in_position:
                        ...
                        # 檢查出場條件
                        if row['Close'] >= row['Middle_BB']: # 碰到中軌，停利
                            ...
                            in_position = False
                        elif row['Close'] <= buy_price * 0.9: # 虧損 10%，停損
                            ret = (row['Close'] - buy_price) / buy_price
                ...
                if len(trades) > 0:
                    ...
                    col3.metric("🔄 過去 5 年觸發次數", f"{total_trades} 次")
                    
                    st.markdown("#### 📜 詳細交易對帳單")
                    # 把小數點報酬率轉換成百分比顯示，方便閱讀
                    trades_df['報酬率'] = trades_df['報酬率'].apply(lambda x: f"{x*100:.2f}%")
                    st.dataframe(trades_df, use_container_width=True)
                else:
                    st.warning("😅 在過去 5 年內，這檔股票【從來沒有】同時符合您左側設定的所有嚴格條件。建議您試著放寬左側的參數（例如調降成交量、放寬 KD 範圍），再回測一次看看！")
